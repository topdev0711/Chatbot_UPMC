...
suite('Out-Of-Pocket Tests: Individual level 1 + level 2 + Out-of-network', function () {
    ...
    test(oopRandomQuestion.text, async function () {
        ...
        validate.answer(getSecondAnswer.followUpQuestion[0], expectedAnswer.answers.thirdLevelQuestion);
        validate.done();

        const clickYesButton2 = await user.conversation.postMessage(action.yes, userData.consumerProfile, this);
        
        validate.statusCode(clickYesButton2.response.status, 200);
        validate.jsonSchema(clickYesButton2.json, postSchema.postMessage);
        validate.done();

        const getThirdAnswer = await user.conversation.getActivity(clickYesButton2.json.id);
        
        validate.statusCode(getThirdAnswer.response.status, 200);
        validate.jsonSchema(getThirdAnswer.json, answerSchema.answer);
        validate.answer(getThirdAnswer.answer, expectedAnswer.answers.thirdLevelAnswer);
        validate.followUpQuestions(getThirdAnswer.followUpQuestion, expectedAnswer.answers.followUpQuestion);
        validate.done();
    });
