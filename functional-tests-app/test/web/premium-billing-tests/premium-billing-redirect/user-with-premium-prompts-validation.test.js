...
suite('Premiums Tests: user with premium (Prompts validation)', function () {
    ...
    test(premiumBillingRandomQuestion.text, async function () {
        ...
        const expectedAnswer = await builder.premiumBillingAnswer(userData, 'premium billing');
    ...
    test(generalPremiumBillingRandomQuestion.text, async function () {
        ...
        const getGeneralPrompts = await user.conversation.getActivity(postMessage.json.id);

        validate.statusCode(getGeneralPrompts.response.status, 200);
        validate.jsonSchema(getGeneralPrompts.json, answerSchema.premiumBillingAnswerWithPrompts);
        validate.answer(getGeneralPrompts.answer, expectedAnswer.answers.generalQuestion);
        validate.buttons(getGeneralPrompts.buttons, expectedAnswer.buttons.generalPrompts);

        const clickPayMedicalBill = await user.conversation.postMessage(action.payMedicalBill, userData.consumerProfile, this);
        
        validate.statusCode(clickPayMedicalBill.response.status, 200);
        validate.jsonSchema(clickPayMedicalBill.json, postSchema.postMessage);
        validate.done();

        const getMedicalPrompts = await user.conversation.getActivity(clickPayMedicalBill.json.id);
        ...
        validate.answer(getMedicalPrompts.answer, expectedAnswer.answers.payMedicalBillQuestion);
