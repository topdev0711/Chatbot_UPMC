...
suite('Live Chat Tests: Transition to Flexible spending account (2 questions)', function () {
    ...
    test(questions.agent.text, async function () {
        ...
        validate.done();
        //////
        const postSecondMessage = await user.conversation.postMessage(questions.iNeedAssistance, userData.consumerProfile, this);
        ...
        const selectTopic = await user.conversation.postMessage(liveChat.flexibleSpendingAccount, userData.consumerProfile, this);
        ...
        validate.answer(getTopicReponse.transitionData.selectedSubject, liveChat.flexibleSpendingAccount.text);
        validate.answer(getTopicReponse.transitionData.selectedSubTopic, '');
        validate.buttons(getTopicReponse.transitionData.questionsBeforeTransition, [`${userName}: ${questions.escalation.text}`, `${userName}: ${questions.iNeedAssistance.text}`]);
        validate.done();    
    });
