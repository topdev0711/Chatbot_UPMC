...
let userData;
let userId; 
const casualConversationsRandomQuestion = randomizer.getRandomQuestion(questions.differentResponseExpectationQuestionsArray);

suite('Casual conversation: Question with different response expectation', function () {
    ...
    test(casualConversationsRandomQuestion.text, async function () {
        ...
        const expectedAnswer = await builder.casualConversationsAnswer('question with different response expectation');
