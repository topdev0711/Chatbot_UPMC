...
suite('Prompts Tests: Spending Account -> Flex Spend Card (PBP H3907-052 (Annual))', function () {
    ...
    test("Spending Accounts -> Flex Spend Card (PBP H3907-052 (Annual))", async function () {
        
        const expectedAnswer = await builder.promptsAnswer(userData, 'common', 'spending accounts');
        const expectedSubAnswer = await builder2.spendingAccountsAnswer(userData, 'active', 'medicare-annual');
