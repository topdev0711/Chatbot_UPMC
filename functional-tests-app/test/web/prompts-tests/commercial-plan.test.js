...
import * as action2 from '../../../test-data/prompts/yes-no-actions.test-data.js';
...
suite('Prompts Tests: Commercial numberman plan Coverage Status', function () {
    ...
    test("Coverage Status -> Dental", async function () {
        ...
        const expectedAnswer = await builder.promptsAnswer(userData, 'common', 'coverage status');
        const expectedSubAnswer = await builder2.eligibilityAnswer(userData, question.dental.text, 'active');
    ...
    test("ID Cards -> Dental", async function () {
        
        const expectedAnswer = await builder.promptsAnswer(userData, 'common', 'idCards');
        const expectedSubAnswer = await builder3.idCardsAnswer(userData, 'active', 'has id card', question.dental.text);
        const postMenu = await user.conversation.postMessage(question.menu, userData.consumerProfile, this);
    ...
    test("ID Cards -> Vision", async function () {
        ...
        const expectedAnswer = await builder.promptsAnswer(userData, 'common', 'idCards');
        const expectedSubAnswer = await builder3.idCardsAnswer(userData, 'active', 'has id card', question.vision.text);
        const postMenu = await user.conversation.postMessage(question.menu, userData.consumerProfile, this);
    ...
    test("Medical Coverage -> Deductible", async function () {
        ...
        const clickYesButton = await user.conversation.postMessage(action2.yes, userData.consumerProfile, this);
        ...
        validate.jsonSchema(getSecondAnswer.json, answerSchema.deductibleOopAnswer);
        validate.answer(getSecondAnswer.answer, expectedSubAnswer.answers.secondLevelAnswer);
        validate.followUpQuestions(getSecondAnswer.followUpQuestion, expectedSubAnswer.answers.followUpQuestion);
        validate.done();
    });
