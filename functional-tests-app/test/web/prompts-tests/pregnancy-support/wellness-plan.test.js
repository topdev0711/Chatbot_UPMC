...
suite('Prompts Tests: Wellness plan -> Pregnancy Support is not included in menu', function () {

    suiteSetup(async function () {
        if (environmentVariables.server != 'STAGE') {  
            userId = environmentVariables.user.doejohna; 
    ...
    test("Pregnancy Support is not included in menu", async function () {
        ...
        validate.doesNotInclude(getFirstMessage.buttons, questions.pregnancySupport.text);
        validate.done();
    });
