...
suite('Prompts Tests: Commercial numberman plan Spending Accounts -> HSA', function () {
    ...
    test("Spending Accounts -> HSA", async function () {
        
        const expectedAnswer = await builder.promptsAnswer(userData, 'common', 'spending accounts');
        const expectedSubAnswer = await builder2.spendingAccountsAnswer(userData, 'hsa');
        ...
        const clickSecondLevelPrompt = await user.conversation.postMessage(question.hsa, userData.consumerProfile, this);
