...
suite('Prompts Tests: Spending Account -> Shop Healthy Card (PBP H4279-004)', function () {
    ...
    test("Spending Account -> Shop Healthy Card (PBP H4279-004)", async function () {
        
        const expectedAnswer = await builder.promptsAnswer(userData, 'common', 'spending accounts');
        const expectedSubAnswer = await builder2.spendingAccountsAnswer(userData, 'active', 'snp');
        ...
        const clickSecondLevelPrompt = await user.conversation.postMessage(question.shopHealthyCard, userData.consumerProfile, this);
        ...
        validate.answer(getAnswer.answersArray[2], expectedSubAnswer.answers.answer[2]);
        validate.answer(getAnswer.answersArray[3], expectedSubAnswer.answers.answer[3]);
