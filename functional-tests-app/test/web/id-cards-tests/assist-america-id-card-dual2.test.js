...
suite.skip('ID Card Tests: User with Assist America ID Card (Dual 2) || Skip reason: dual plan test issue. Investigation is required', function () {

    suiteSetup(async function () {
        userId = environmentVariables.user.jesslecker2; 
    ...
    test(dentalDiscountIdCardRandomQuestion.text, async function () {
        ...
        validate.statusCode(getNoActionResponse.response.status, 200);
        validate.jsonSchema(getChatBotResponse.json, answerSchema.noAvailableIdCardWithPrompt);
        validate.followUpQuestions(getNoActionResponse.followUpQuestion, expectedIdCardAnswer.answers.secondFollowUpQuestion);
        validate.done();
    });
