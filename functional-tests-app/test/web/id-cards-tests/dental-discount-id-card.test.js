...
suite('ID Card Tests: User with Dental Discount ID Card', function () {

    suiteSetup(async function () {
        userId = environmentVariables.user.chatmarcela;
    ...
    test(generalIdCardRandomQuestion.text, async function () {
        ...
        const expectedIdCardAnswer = await builder.idCardsAnswer(userData,  'active', 'has id card', generalIdCardRandomQuestion.text, idCard.idCardsIds.dentalDiscount);
        ...
        const clickIdCardButton = await user.conversation.postMessage(idCard.dentalDiscountIdCard, userData.consumerProfile, this);
        ...
        validate.jsonSchema(getChatBotResponse.json, answerSchema.clickDdIdCardWithPrompt);
    ...
    test(dentalIdCardRandomQuestion.text, async function () {
        ...
        validate.statusCode(getChatBotResponse.response.status, 200);
        validate.jsonSchema(getChatBotResponse.json, answerSchema.ddIdCardInsteadOfDIdCard);
        validate.answer(getChatBotResponse.answersArray[0], expectedIdCardAnswer.answers.dentalDiscountAnswer);
        validate.answer(getChatBotResponse.answersArray[1], expectedIdCardAnswer.answers.hereIsYourIdCardAnswer);
        validate.url(getChatBotResponse.urls, expectedIdCardAnswer.idCardUrl);
        validate.answer(getChatBotResponse.answersArray[2], expectedIdCardAnswer.answers.thirdFollowUpQuestion);
        validate.buttons(getChatBotResponse.buttons, action.buttons);
        validate.done();
