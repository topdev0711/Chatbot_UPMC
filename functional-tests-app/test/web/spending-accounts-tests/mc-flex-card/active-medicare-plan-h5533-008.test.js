...
const trnRandomQuestion = randomizer.getRandomQuestion(questions.trnQuestionsArray);

suite('Spending Account (MC Flex Card) Tests: Active Medicare Plan with PBP H5533-008 (Rewards)', function () {
    ...
    test(flexSpendCardRandomQuestion.text, async function () {
        ...
        const expectedAnswer = await builder.spendingAccountsAnswer(userData, 'medicare-rewards');
    ...
    test(trnRandomQuestion.text, async function () {
        
        const expectedAnswer = await builder.spendingAccountsAnswer(userData, 'no requested account');
        const expectedAnswer2 = await builder.spendingAccountsAnswer(userData, 'medicare-rewards');
        const postMessage = await user.conversation.postMessage(trnRandomQuestion, userData.consumerProfile, this);
        ...
        const selectSpendingAccount = await user.conversation.postMessage(action.flexSpendCard, userData.consumerProfile, this);
