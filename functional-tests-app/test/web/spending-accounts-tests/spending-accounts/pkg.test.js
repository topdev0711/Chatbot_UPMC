...
const pkgRandomQuestion = randomizer.getRandomQuestion(questions.pkgQuestionsArray);
...
suite('Spending Account Tests: PKG', function () {

    suiteSetup(async function () {
        userId = environmentVariables.user.pkgtestmem2;
        userData = await collector.getData(userId, ['eligibility', 'medicareFlexCard', 'spendingAccounts']);      
        await user.startChat(userData.userToken, userData.consumerProfile, 'web', this);
    });

    test(pkgRandomQuestion.text, async function () {
        
        const expectedAnswer = await builder.spendingAccountsAnswer(userData, 'pkg');
        const postMessage = await user.conversation.postMessage(pkgRandomQuestion, userData.consumerProfile, this);
    ...
    test(hsaRandomQuestion.text, async function () {
        ...
        const expectedAnswer2 = await builder.spendingAccountsAnswer(userData, 'pkg');
