...
suite('Spending Account Tests: HRA & HIA', function () {

    suiteSetup(async function () {
        userId = environmentVariables.user.cynthsma;
    ...
    test(hraRandomQuestion.text, async function () {
        ...
        const expectedAnswer = await builder.spendingAccountsAnswer(userData, 'hra');
        ...
        const selectSpendingAccount = await user.conversation.postMessage(action.hra, userData.consumerProfile, this);
    ...
    test(fsaRandomQuestion.text, async function () {
        ...
        const expectedAnswer2 = await builder.spendingAccountsAnswer(userData, 'hia');
