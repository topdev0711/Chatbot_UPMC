...
suite('PCP Tests: intents recognition', function () {

    suiteSetup(async function () {
        userId = environmentVariables.user.ahaines;
        userData = await collector.getData(userId, ['pcp']);      
        await user.startChat(userData.userToken, userData.consumerProfile, 'web', this);
    });
    
    questions.currentPcpQuestionsArray.forEach((question) => {
        test(question.text, async function () {
            
            const expectedAnswer = await builder.currentPcpAnswer(userData, 'active/future', 'current pcp', environmentVariables.domain);
