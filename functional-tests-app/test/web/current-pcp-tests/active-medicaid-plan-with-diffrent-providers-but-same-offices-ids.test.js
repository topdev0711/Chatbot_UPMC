...
suite('PCP Tests: Active Medicaid plan with different providers but same offices ids', function () {

    suiteSetup(async function () {
        userId = environmentVariables.user.lancgeo;
    ...
    test(currentPcpRandomQuestion.text, async function () {
        ...
        const expectedAnswer = await builder.currentPcpAnswer(userData, 'active/future', 'different providers with same officeId', environmentVariables.domain);
        ...
        validate.answer(getAnswer.answersArray[3], expectedAnswer.answer[3]);
        validate.answer(getAnswer.answersArray[4], expectedAnswer.answer[4]);
