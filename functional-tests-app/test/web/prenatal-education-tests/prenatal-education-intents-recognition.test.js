...
suite('Prenatal Education Tests: intents recognition', function () {

    suiteSetup(async function () {
        userId = environmentVariables.user.tstmem15;
        userData = await collector.getData(userId);      
        await user.startChat(userData.userToken, userData.consumerProfile, 'web', this);
    });

    questions.prenatalEducationAssessmnetFormQuestionsArray.forEach((question) => {
        test(question.text, async function () {
            
            const expectedAnswer = await builder.prenatalEducationAssessmentFormAnswer(userData, 'form');
