...
suite('Eligibility Tests: Active Commercial OEXC plan with future term date and no future plan', function () {

    suiteSetup(async function () {
        userId = environmentVariables.user.chatmykhaylo;
        userData = await collector.getData(userId, ['eligibility', 'visionDiscountSchedule']); 
    ...
    test(visionEligibilityRandomQuestion.text, async function () {
        ...
        if (getAnswer.urls.length > 0) {
            const file = await user.conversation.getFile(getAnswer.urls);
        
            validate.answer(file, expectedAnswer.fileData);
            validate.done();
        }
    });
