...
suite('Eligibility Tests: Active Commercial plan with Active Commercial, Madicaid and termed Commercial plans on dual coverage', function () {

    suiteSetup(async function () {
        userId = environmentVariables.user.kurni;
        userData = await collector.getData(userId, ['eligibility', 'dualInfo']);      
        await user.startChat(userData.userToken, userData.consumerProfile, 'web', this);
    });

    test(generalEligibilityRandomQuestion.text, async function () {
        
        const expectedAnswer = await builder.eligibilityAnswer(userData, generalEligibilityRandomQuestion.text, 'dual coverage');
        ...
        validate.answer(getChatBotResponse.answersArray[0], expectedAnswer.answer[0]);
        validate.answer(getChatBotResponse.answersArray[1], expectedAnswer.answer[1]);
    ...
    test(dentalEligibilityRandomQuestion.text, async function () {
        ...
        const expectedAnswer = await builder.eligibilityAnswer(userData, dentalEligibilityRandomQuestion.text, 'dual coverage');
        const expectedLiveChatAnswer = await builder2.liveChatAnswer(userData);
    ...
    test(visionEligibilityRandomQuestion.text, async function () {
        ...
        const expectedAnswer = await builder.eligibilityAnswer(userData, visionEligibilityRandomQuestion.text, 'dual coverage');
        const expectedLiveChatAnswer = await builder2.liveChatAnswer(userData);
    ...
    test(wellnessEligibilityRandomQuestion.text, async function () {
        ...
        const expectedAnswer = await builder.eligibilityAnswer(userData, wellnessEligibilityRandomQuestion.text, 'dual coverage');
        const expectedLiveChatAnswer = await builder2.liveChatAnswer(userData);
